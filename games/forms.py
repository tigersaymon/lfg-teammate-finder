...
class UserGameProfileForm(forms.ModelForm):
    ...
    def __init__(self, *args, user: Any = None, **kwargs) -> None:
        ...
        # Logic to restrict game selection for new profiles
        if user and not self.instance.pk:
            ...
            # if editing, cannot change the game
        ...
        # Dynamic role queryset form main_role list (HTMX)
        if "game" in self.data:
            try:
                game_id = int(self.data.get("game"))  # If POST, use the selected game id
                self.fields["main_role"].queryset = GameRole.objects.filter(game_id=game_id)
            except (ValueError, TypeError):
                pass

        elif self.instance.pk:
            # if editing, roles associated with the profile game
